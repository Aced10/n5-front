...
export default function EnhancedTable() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = permissions.map((n) => n.name);
  ...
  const handleClickEdit = (event, permission) => {
    setMode(false);
    setPermission(permission);
    handleShow();
  };

  const handleClickCreate = (event, permission) => {
    setMode(true);
    setPermission(permission);
    handleShow();
  };
  ...
  return (
    <Box sx={{ width: "100%" }}>
            ...
            <TableBody>
              {stableSort(permissions, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow hover tabIndex={-1} key={row.PermissionID}>
                      <TableCell component="th" id={labelId} scope="row">
                        {row.EmployeeFirstName} {row.EmployeeLastName}
                      </TableCell>
                      <TableCell>{row.Description}</TableCell>
                      <TableCell>{row.PermissionDate}</TableCell>
                      <TableCell>
                        <IconButton
                          aria-label="delete"
      ...
      <Button onClick={handleClickCreate}>AGREGAR PERMISO</Button>
    </Box>
  );
}
